...
export default function WishlistPageClient({ initialItems }: WishlistPageClientProps) {
  ...
  const {
    ...
    total,
    loadMore,
  } = useWishlist({ initialData: initialItems });
  const { addVinyl } = useVinyls({ autoFetch: false });
  ...
  const handleEditClick = useCallback((item: WishlistItem) => {
    setEditingItem(item);
    setDiscogsInitialData(null);
    setIsFormOpen(true);
  }, []);
  ...
  const handleAddToCollectionClick = useCallback((item: WishlistItem) => {
          ...
          <div className={`flex flex-col sm:flex-row sm:items-center gap-4 ${isOwner ? 'sm:justify-between' : 'sm:justify-center text-center'}`}>
            <div>
              ...
              <p className="mt-1 text-steel-400">
                {total || items.length} {(total || items.length) === 1 ? 'item' : 'items'} on your wishlist
              </p>
            </div>
            {isOwner && (
